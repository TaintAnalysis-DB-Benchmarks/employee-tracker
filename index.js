...
const viewEmployees = async () => {
  ...
  const roles_f4xg = await Role.findAll({
    where: {
      id: employees.map(data => data.role_id)
    },
    raw: true
  });
  const employees_pcwx = await Employee.findAll({
    where: {
      id: employees.map(data => data.manager_id)
    },
    raw: true
  });
  ...
  for (i = 0; i < employeeList.length; i++) {
    ...
    let roleName = roles_f4xg.filter(x => x.id === employeeList[i].role_id); // Get the manager name to replace the role ID in the table

    let managerName = employees_pcwx.filter(x => x.id === employeeList[i].manager_id); // Get the department of the specific role
...
const start = async () => {
  ...
  inquirer.prompt([{
    ...
    choices: ["View All Employees", "View All Departments and Their Employees", "View All Roles", "Add Employee", "Add Role", "Add Department", "Remove Employee", "Remove Department", "Remove Role", "Update Employee Role", "Update Employee Manager"]
  }]).then(answer => {
    let chosen = answer.mainQuestion; // Runs a switch statement based on the prompt given beforehand
...
const allEmployee = async () => {
  let employeeList = []; // Simply gets the employees and puts them in an array
...
const allDepartment = async () => {
  let departmentList = []; // Gets the departments for the prompt to display
  ...
  inquirer.prompt([{
    type: "list",
    name: "mainQuestion",
    message: "Select a department to view all the employees under that department.",
    ...
    if (answer.mainQuestion === "Return To Main Menu") {
      ...
      const employees_dgtc = await Employee.findAll({
        where: {
          role_id: rolesChosen.map(data => data.id)
        },
        raw: true
      });
      ...
      for (i = 0; i < rolesChosen.length; i++) {
        ...
        const employeesChosen = employees_dgtc.filter(x => x.role_id === rolesChosen[i].id); // Goes through the list of employees and pushes their names to an array
...
const addRole = async () => {
  ...
  inquirer.prompt([{
    ...
    departmentName = answer.roleDepartment; // Get the role department by name so we can get that id
...
const removeRole = async () => {
  ...
  inquirer.prompt([{
    ...
    if (answer.roleChoice === "Go Back To Main Menu") {
      ...
      let roleName = answer.roleChoice; // Destroys selected role
...
const removeDepartment = async () => {
  ...
  inquirer.prompt([{
    ...
    if (departmentName === "Return To Main Menu") {
      ...
      console.log(rolesInDepartment); // Check to see if they align
...
const addEmployee = async () => {
  ...
  managerList.push("They are a manager"); // Get all the roles
  ...
  inquirer.prompt([{
    ...
    managerName = answer.employeeManager; // Determine if the employee is a manager or not
...
const updateRole = async () => {
  let employeeList = [];
  let roleList = []; // Get all the employees
